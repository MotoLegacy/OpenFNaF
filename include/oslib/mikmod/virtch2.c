...
static void Mix32To16_Stereo(SWORD *dste, SLONG *srce, SLONG count)
{
    ...
    {   tmpx = tmpy = 0;
		...
		tmp1 = tmpx / SAMPLING_FACTOR;
		tmp2 = tmpy / SAMPLING_FACTOR;
        *dste++ = tmp1;
        *dste++ = tmp2;
		if (md_mixshift == 1)		{
			*dste++ = tmp1;
...
static void Mix32To8_Normal(SBYTE *dste, SLONG *srce, SLONG count)
{
    ...
    {   tmpx = 0;
		...
		tmp = (tmpx / SAMPLING_FACTOR) + 128;
        *dste++ = tmp;
		if (md_mixshift == 1)
	        *dste++ = tmp;
		else if (md_mixshift == 2)		{
	        *dste++ = tmp;
	        *dste++ = tmp;
		}
    }
...
static void Mix32To8_Stereo(SBYTE *dste, SLONG *srce, SLONG count)
{
    ...
    {   tmpx = tmpy = 0;
        ...
        {   x1 = *srce++ / (MAXVOL_FACTOR * 256);
 		...
 		tmp1 = (tmpx / SAMPLING_FACTOR) + 128;
 		tmp2 = (tmpy / SAMPLING_FACTOR) + 128;
        *dste++ = tmp1;
        *dste++ = tmp2;
		if (md_mixshift == 1)		{
	        *dste++ = tmp1;
...
void VC2_WriteSamples(SBYTE *buf, long todo)
{
    ...
    {   if(TICKLEFT==0)
        ...
        buf += (samples2bytes(left) / SAMPLING_FACTOR) << md_mixshift;
        ...
        {   portion = MIN(left, samplesthatfit);
            ...
            buffer += (samples2bytes(portion) / SAMPLING_FACTOR) << md_mixshift;
            left   -= portion;
        }
    }
}
