...
void SL_Load(void *buffer, SAMPLOAD *smp, int length)
{
    ...
    {   stodo = (length<2048) ? length : 2048;
        ...
        {   SBYTE  *s;
            ...
            _mm_fread(sl_buffer,sizeof(SBYTE),stodo,smp->fp);
...
void SL_LoadStream(void *buffer, UWORD infmt, UWORD outfmt, int length, VIRTUAL_FILE *fp)
...

{
    ...
    {   stodo = (length<2048) ? length : 2048;
        ...
        {   SBYTE  *s;
            ...
            _mm_fread(sl_buffer,sizeof(SBYTE),stodo,fp);
...
SAMPLOAD *SL_RegisterSample(SAMPLE *s, int type, VIRTUAL_FILE *fp)    /* Returns 1 on error! */
