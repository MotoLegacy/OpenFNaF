...
// A faire pour supporter md_mixshift
...
static void AddChannel(SLONG *ptr, SLONG todo)
{
    ...
    {   // update the 'current' index so the sample loops, or
        ...
        {
#ifdef __ASSEMBLY__
            if(md_mode & DMODE_INTERP) //&& ((vnf->increment >> (FRACBITS+1)) == 0))
...
#else
            if((md_mode & DMODE_INTERP))      // && ((vnf->increment & ((1ul<<30)-1)) < (FRACMASK<<1)))
            {   if(vc_mode & DMODE_STEREO)
                    if((vnf->pan == PAN_SURROUND) && (vc_mode & DMODE_SURROUND))
...
void VC_WriteSamples(SBYTE *buf, ULONG todo)
{
    ...
    {   if(TICKLEFT==0)
        ...
        buf += samples2bytes(left) << md_mixshift;
        ...
        {   portion = MIN(left, samplesthatfit);
            ...
            buffer += samples2bytes(portion) << md_mixshift;
            left   -= portion;
        }
    }
}
