...
BOOL ReadComment(UWORD len)
{
    ...
    {   if(!(of.comment=(CHAR *)_mm_malloc(len+1))) return 0;
        _mm_fread(of.comment,len,1,modfp);
        of.comment[len] = 0;
    }
    return 1;
}
...
CHAR *MikMod_LoadSongTitle(CHAR *filename)
{
    ...
    if(l==NULL)
    {   _mm_errno = MMERR_NOT_A_MODULE | 0x3000;
        _mm_iobase_revert();
...
UNIMOD *MikMod_LoadSongFP(VIRTUAL_FILE *fp, int maxchan)
...

{
    ...
    if(l==NULL)
    {   _mm_errno = MMERR_NOT_A_MODULE | 0x1000;
        _mm_iobase_revert();
        if(_mm_errorhandler!=NULL) _mm_errorhandler();
        return NULL;
    }
...
UNIMOD *MikMod_LoadSongVF(VIRTUAL_FILE *fp, int maxchan)

/* Loads a module given a VIRTUAL_FILE. */
/* File is loaded from the current file seek position. */

{
    ...
    if(l==NULL)
    {   _mm_errno = MMERR_NOT_A_MODULE | 0x2000;
        _mm_iobase_revert();
        if(_mm_errorhandler!=NULL) _mm_errorhandler();
        return NULL;
    }
